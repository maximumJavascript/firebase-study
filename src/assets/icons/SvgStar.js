export function StarIcon(props) {
  if (!props.filled || props.filled < 0) {
    return <StarIconSvg className={props.className} fill={'transparent'} />;
  } else if (props.filled >= 1) {
    return <StarIconSvg className={props.className} fill={'currentColor'} />;
  } else {
    const percentage = `${props.filled * 100}%`;

    return (
      <StarIconSvg className={props.className} fill="url(#grad)">
